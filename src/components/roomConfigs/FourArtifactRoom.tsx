...
export const FourArtifactRoom = () => {
    const { getRoomDepth, roomWidth, artifacts } = useZeumStore();
    const roomDepth = getRoomDepth();
    return (
        <>
            {/* Back Wall */}
            <Wall position={[-12.5, 7.5, 38]} horizontal width={2} height={15} depth={75} />
            ...
            <Wall position={[-49, 2.5, 26]} width={2} height={5} depth={roomDepth / 3} />
            <Wall position={[-37.5, 2.5, 14]} horizontal width={2} height={5} depth={25} visible={false} />

            <Floor position={[-36.5, 0, 26]} width={roomWidth / 1.85} depth={roomDepth / 3} />
            <MainFloor height={roomDepth} width={roomWidth} />

            <DisplayWall artifact={artifacts[3]} position={[-35, 4, 35]} width={10} height={8} depth={0.5} />
